...
Srinivas Atreya [srinivas.atreya@gmail.com]
"""
...
def square_distance(pointA, pointB):
    ...
    for dimension in range(dimensions):
        distance += (int(float(pointA[dimension])) - int(float(pointB[dimension])))**2
    return distance
...
class KDTree():
    ...
    def query(self, query_point, t=1):
        ...
        def nn_search(node, query_point, t, depth, best_neighbours):
            ...
            if (int(float(node.point[axis])) - int(float(query_point[axis])))**2 < best_neighbours.largest_distance:
